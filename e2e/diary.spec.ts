...
test.describe.serial('Diary journey', () => {
	...
	test('create an entry', async ({ page }) => {
		...
		await page
			.getByLabel('Song')
			.fill('https://open.spotify.com/track/7mCI9JBW0FJAaMk3H3TFyo?si=d3fc294d50cc467f');
		await page
			.getByLabel('Content')
			.fill('This is my first entry and i really remember this song.');
		await page.getByLabel('Date').fill(new Date().toISOString().substring(0, 10));
		await page.getByRole('button', { name: 'Create' }).click();

		await expect(page.getByText(entryTitle)).toBeVisible();
		await expect(
			page.getByText('This is my first entry and i really remember this song.')
		).toBeVisible();
	});

...
export const mapSingleEntry = (rows: EntryRecord[]): Entry => {
	...
	const images = rows
		...
		.map(
			(r) =>
				new AttachedImage(new ImageId(r.imageId!), new URL(`http://localhost:5173/${r.imageId}`))
		);
	...
	return Entry.builder()
		...
		.withImages(...images)
		.content(row['content'])
		.song(new SpotifySong(new SpotifyId(row['songId']), new SpotifyURL(row['url']), row['embed']))

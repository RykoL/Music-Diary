...
export class DiaryRepository implements DiaryPersistence, EntryPersistence {
	...
	async saveEntry(newEntry: Entry) {
		...
		try {
			...
			await Promise.all(
				newEntry.getUnAttachedImages().map((img) => {
	...
	async getDiaries(id: UserId): Promise<Array<Diary>> {
		const records = await this.db.all<Array<DiaryRecord>>(
			'SELECT id as diaryId, title as diaryTitle, description as diaryDescription from diary where owner_id = ?;',
			[id.value]
		);
	...
	async saveDiary(diary: Diary) {
		await this.db.run('INSERT INTO diary VALUES (?, ?, ?, ?);', [
			diary.id.value,
			diary.title,
			diary.description,
			diary.owner.value
